...
If the variable name matches the prop name exactly, Svelte has a shorthand you'll use
constantly once you start composing components:
...
You'll want this pattern once you build something like `Button` or `NavLink`: a component that
renders differently depending on a `variant` prop, using a plain object as a lookup table
instead of an `if`/`else if`/`else` chain:
...
You may wonder whether a component's text should be a prop —
`<Button label="Book now" href="#book" />` — or written the more HTML-like way, between its
tags: `<Button href="#book">Book now</Button>`.
...
everything above. For this assignment, every piece of button/badge-style text in the design is
plain text — no icon, no inline markup — so a plain `label`/`text` prop does everything you
need; prefer that over reaching for children/snippets here. If you're curious, Svelte's docs
cover snippets and children under "Svelte > Template syntax" — not required for Assignment 1.
...
We'll properly cover runes and reactivity in the next lesson. For Assignment 1, when you reach
the appointment-type dropdown, it's fine to treat `$state`/`$derived` as "this is how Svelte
remembers things" without understanding every rule behind them yet — the assignment
deliberately suggests building this component last for that reason.

One thing you might hit: if the dropdown's options come in as a prop (say, `types`), writing
`$state(types[0].id)` to seed the initial selection makes Svelte print a
`state_referenced_locally` warning — it's flagging that this only reads `types`' *initial*
value, which here is exactly what you want. Wrapping it as
`$state(untrack(() => types[0].id))` (import `untrack` from `'svelte'`) tells Svelte that's
intentional. It's a minor, one-line escape hatch for this specific situation, not a concept to
dig into further right now.
